...
class CQLPolicy:
    def __init__(self, Q1, Q2, Q3, scalers):
        self.nets    = {0: Q1, 1: Q2, 2: Q3}
...
def main():
    ...
    scalers = {0: ckpt['scaler1'], 1: ckpt['scaler2'], 2: ckpt['scaler3']}
    policy  = CQLPolicy(load_net(0), load_net(1), load_net(2), scalers)
    params  = load_pickle(f"data/multi_cql_{suffix}_{args.n_cases}_params.pkl")

    print(f"Evaluating Multi-Model CQL — {suffix}")
    ...
    results = {'Bank': bank_res, 'Random': random_res, f'CQL-MM {suffix}': cql_res}
