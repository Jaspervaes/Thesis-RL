"""Convert raw SimBank data to RIMS transitions with prefix sequences and timing targets."""
import sys
import os
import argparse
import numpy as np
...
def extract_transitions(df, steps=3):
    ...
    for _, group in df.groupby('case_nr'):
        ...
        def timing(i_idx):
            ts_i  = group.loc[i_idx, 'timestamp']
            proc  = (ts_i - group.loc[i_idx - 1, 'timestamp']).total_seconds() / 3600
        ...
        if has1 and has2:
            ...
            proc1, rem1 = timing(i1)
            proc2, rem2 = timing(i2)
            rows += [
                {'prefix': p0, 'action': a0, 'reward': 0.0,    'next_prefix': p1, 'terminal': False, 'intervention': 0, 'next_intervention': 1, 'proc_time': proc0, 'rem_time': rem0},
                {'prefix': p1, 'action': a1, 'reward': 0.0,    'next_prefix': p2, 'terminal': False, 'intervention': 1, 'next_intervention': 2, 'proc_time': proc1, 'rem_time': rem1},
                {'prefix': p2, 'action': a2, 'reward': outcome, 'next_prefix': [], 'terminal': True,  'intervention': 2, 'next_intervention': -1, 'proc_time': proc2, 'rem_time': rem2},
            ]
            ...
            proc2, rem2 = timing(i2)
            rows += [
                {'prefix': p0, 'action': a0, 'reward': 0.0,    'next_prefix': p2, 'terminal': False, 'intervention': 0, 'next_intervention': 2, 'proc_time': proc0, 'rem_time': rem0},
                {'prefix': p2, 'action': a2, 'reward': outcome, 'next_prefix': [], 'terminal': True,  'intervention': 2, 'next_intervention': -1, 'proc_time': proc2, 'rem_time': rem2},
            ]
