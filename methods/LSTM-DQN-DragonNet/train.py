...
def main():
    ...
    # Synthesize one extra transition per alternative action via per-action outcome
    # heads. predict_outcomes() routes through head a when called with constant-a
    # actions. Non-terminal transitions are not augmented.
    # Expected buffer size ~= #terminal * N_ACTIONS[int_idx] + #non-terminal.
    print(f"\n{'='*50}")
