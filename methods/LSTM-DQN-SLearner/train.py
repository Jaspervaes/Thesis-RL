...
def main():
    ...
    # Phase 2: Counterfactual-augmented causal rewards (LSTM S-learners)
    # Synthesizes one extra transition per alternative action via model.predict(prefix, a).
    # Non-terminal transitions are not augmented.
    # Expected buffer size ~= #terminal * N_ACTIONS[int_idx] + #non-terminal.
    print(f"\n{'='*50}")
    print("Phase 2: Counterfactual augmentation via LSTM S-learners")
    print('='*50)
    ...
    for int_idx, slearner in slearners.items():
        ...
        for df_c, synth_list, label in [
            ...
            # (1) Factual leg — replace observed reward in place.
