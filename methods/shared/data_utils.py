...
_INT_NAMES = ["choose_procedure", "time_contact_HQ", "set_ir_3_levels"]


def active_tag(active):
    """Model/result tag — always unique: [0,1,2] -> '_active012', [0,2] -> '_active02'."""
    return "_active" + "".join(str(i) for i in sorted(active))


def data_tag(active):
    """Data file tag — [0,1,2] reuses the standard CONF dataset (no suffix)."""
    return "" if sorted(active) == [0, 1, 2] else "_active" + "".join(str(i) for i in sorted(active))


def generate_rct_data_subset(n_cases, seed, active):
    """Pure-RCT data with only `active` interventions randomised; inactive follow bank policy.

    Mirrors generate_confounded_data_subset but without bank/RCT delta mixing — single
    simulation run with RCT=True and active_names restricting which interventions are
    randomised. Inactive interventions fall back to the configured bank policy inside
    SimBank, matching deployment where the trained policy only overrides the active set.
    """
    active_names = [_INT_NAMES[i] for i in sorted(active)]
    params = get_simbank_params(n_cases, seed, rct=True)
    params["intervention_info"]["active_names"] = active_names
    gen = simulation.PresProcessGenerator(params, seed)
    df = pd.DataFrame(gen.run_simulation_normal(n_cases))
    return df, params


def generate_confounded_data_subset(n_cases, seed, active, delta=0.95):
    """Confounded data with only `active` interventions randomised; inactive follow bank policy."""
    active_names = [_INT_NAMES[i] for i in sorted(active)]
    ...
    rct_seed = seed * 10
    params_rct = get_simbank_params(n_cases, rct_seed, rct=True)
    params_rct["intervention_info"]["active_names"] = active_names
    params_rct["simulation_start"] = deepcopy(gen_bank.simulation_end)
    gen_rct = simulation.PresProcessGenerator(params_rct, rct_seed)
    df_rct = pd.DataFrame(gen_rct.run_simulation_normal(n_cases))
