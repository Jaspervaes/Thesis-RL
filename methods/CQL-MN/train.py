"""Train Multi-Model CQL with LSTM-DQN: Q3 → Q2 → Q1 with backward TD + CQL penalty."""
import random
import sys
import os
import argparse
...
def train_q(model, target, opt, tr, va, target_fn, args):
    """Train one Q-network with CQL conservative penalty."""
    ...
    for epoch in range(args.epochs):
        ...
        for b in tr:
            ...
            cql_loss = (torch.logsumexp(q, 1) - q_taken).mean()
...
def main():
    ...
    def loader(df, int_idx, shuffle=True):
        return make_loader(df, int_idx, activity_to_idx, feat_means, feat_stds, max_len, bs, shuffle,
                           seed=args.seed + int_idx, n_activities=n_activities)
