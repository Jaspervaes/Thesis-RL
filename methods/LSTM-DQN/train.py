...
from shared import (
    ...
    build_vocab_and_stats, encode, seed_worker, active_tag, data_tag,
)
...
def main():
    ...
    parser.add_argument('--active',     type=str,   default=None,
                        help='Comma-separated active intervention indices, e.g. "1,2"')
    ...
    if args.active is not None:
        ...
        tag      = active_tag(active)   # model/result files: always _active012 etc.
        dtag     = data_tag(active)     # data files: "" for [0,1,2] (reuse standard regime data)
        suffix   = "CONF" if args.confounded else "RCT"
        base = f"data/lstm_{suffix}_{args.n_cases}{dtag}"
        print(f"Training LSTM-DQN — {suffix} subset active={active} | lr={args.lr} epochs={args.epochs}")
        ...
        r_mean = float(term_r.mean()); r_std = float(term_r.std()) + 1e-8
        def norm(r): return (r - r_mean) / r_std

        cfg = {
            'n_activities': n_activities, 'n_features': len(FEATURE_COLS),
            'feature_cols': FEATURE_COLS, 'activity_to_idx': activity_to_idx,
            'feat_means': feat_means, 'feat_stds': feat_stds, 'max_len': max_len,
            'emb_dim': args.emb_dim, 'hidden': args.hidden, 'n_layers': args.n_layers,
            'dropout': args.dropout, 'n_actions': N_ACTIONS,
            'activity_enc': args.activity_enc,
        }

        def make_model(n_act):
            m  = LSTM_DQN(n_activities, len(FEATURE_COLS), n_act, args.emb_dim, args.hidden,
                          args.n_layers, args.dropout, activity_enc=args.activity_enc).to(device)
            mt = LSTM_DQN(n_activities, len(FEATURE_COLS), n_act, args.emb_dim, args.hidden,
                          args.n_layers, args.dropout, activity_enc=args.activity_enc).to(device)
            mt.load_state_dict(m.state_dict())
            return m, mt

        def ldr(df, int_idx, shuffle=True):
            return make_loader(df, int_idx, activity_to_idx, feat_means, feat_stds, max_len,
                               args.batch_size, shuffle, seed=args.seed + int_idx,
                               activity_enc=args.activity_enc, n_activities=n_activities)

        active_sorted = sorted(active)

        # Init all models upfront in forward order — same RNG consumption regardless of subset
        models_init = {i: make_model(N_ACTIONS[i]) for i in active_sorted}

        # Create all loaders upfront in forward order — same as --steps 3 for [0,1,2]
        loaders_tr = {i: ldr(df_train, i)        for i in active_sorted}
        loaders_va = {i: ldr(df_val,   i, False)  for i in active_sorted}

        def make_td_target(Q_targets_dict):
            # Computes all downstream Q-networks on the full batch, routes by next_intervention.
            # Mirrors --steps 3 tgt1/tgt2 style: full-batch forward, then mask-based routing.
            def tgt(b):
                r, term = b['reward'].squeeze(1).to(device), b['terminal'].squeeze(1).to(device)
                ni = b['next_intervention'].squeeze(1).to(device)
                t = term * norm(r)
                with torch.no_grad():
                    nq_dict = {idx: Qt(b['n_acts'].to(device), b['n_feats'].to(device),
                                       b['n_lens'].squeeze(1))
                               for idx, Qt in Q_targets_dict.items()}
                for idx, nq in nq_dict.items():
                    mask = ((1 - term).bool()) & (ni == idx)
                    if mask.any():
                        t[mask] = args.gamma * nq[mask].max(1)[0]
                return t
            return tgt
        ...
        for i in reversed(active_sorted):
            Q, Qt = models_init[i]
            tr = loaders_tr[i]; va = loaders_va[i]
            print(f"\n[Q_int{i}]")
            if i == active_sorted[-1]:
                target_fn = lambda b: norm(b['reward'].squeeze(1).to(device))
            else:
                later_targets = {j: Q_target[j] for j in active_sorted if j > i}
                for Qt_j in later_targets.values():
                    Qt_j.eval()
                target_fn = make_td_target(later_targets)
            best = train_q(Q, Qt, optim.Adam(Q.parameters(), args.lr, weight_decay=1e-5),
                           tr, va, target_fn, args)
            Q.load_state_dict(best); Qt.load_state_dict(best)
            trained[i] = best
            Q_target[i] = Qt

        save_dict = {f'Q_int{i}': sd for i, sd in trained.items()}
        save_dict['config'] = cfg
        os.makedirs("models", exist_ok=True)
        model_path = f"models/lstm_{suffix}_{args.n_cases}_s{args.seed}{tag}.pth"
