...
from shared import (
    ...
    N_ACTIONS, LSTM_DQN, encode_prefix, active_tag, data_tag,
)
...
def main():
    ...
    if args.active is not None:
        ...
        tag  = active_tag(active)   # model file: always _active012 etc.
        dtag = data_tag(active)     # params/data file: "" for [0,1,2]
        ids  = "".join(str(i) for i in sorted(active))
        suffix = "CONF" if args.confounded else "RCT"
        ckpt   = torch.load(f"models/lstm_{suffix}_{args.n_cases}_s{args.train_seed}{tag}.pth",
                            map_location=device, weights_only=False)
        cfg    = ckpt['config']
        params = load_pickle(f"data/simbank_{suffix}_{args.n_cases}{dtag}_params.pkl")
        ...
        models = {i: load_net(f'Q_int{i}', N_ACTIONS[i]) for i in active}
        policy = LSTMPolicy(models, cfg)
        label  = f'lstm_{suffix}_Int{ids}'

        print(f"Evaluating LSTM-DQN — {suffix} subset active={active}")
        ...
        if args.results_file:
            ...
            with open(args.results_file, 'w') as f:
                json.dump({label: lstm_res['avg'], 'Bank': bank_res['avg']}, f)
        return
