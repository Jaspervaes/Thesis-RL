...
def extract_transitions(df):
    """
    Extract transitions with prefix sequences for each intervention.
    ...
    for _, group in df.groupby('case_nr'):
        ...
        if has1 and has2:
            ...
            rows.append({'prefix': p0, 'action': a0, 'reward': outcome, 'next_prefix': [], 'terminal': True, 'intervention': 0, 'next_intervention': -1})
