...
class LoanProcessPreprocessor():
    ...
    def preprocess_reward_RL(self, reward, prep_utils, device):
        outcome_scaler = prep_utils["scaler_dict_train"]["outcome"]
        tensor_reward = torch.Tensor([[reward]])
        scaled_reward = outcome_scaler.transform(tensor_reward)
        scaled_reward = scaled_reward[0][0]
    ...
    def retain_normal(self, data_padded, data_type="train"):
        ...
        if data_type == "train":
            boolean_retainer = torch.isin(torch.arange(treatment.size(0)), torch.cat((all_zeros_indices, last_element_indices)))
