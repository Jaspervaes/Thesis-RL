...
def compare_marginal_hists(x1, x2, label1=None, label2=None, ax=None):
    # if is_binary(x1, x2):
    #     sns.displot(x1, kde=False, ax=ax, label=label1)
    #     sns.displot(x2, kde=False, ax=ax, label=label2)
    # else:
    #     try:
    #         sns.displot(x1, ax=ax, label=label1)
    #     except RuntimeError:
    #         sns.displot(x1, ax=ax, label=label1, kde_kws={'bw': 0.5})
    #     try:
    #         sns.displot(x2, ax=ax, label=label2)
    #     except RuntimeError:
    #         sns.displot(x1, ax=ax, label=label1, kde_kws={'bw': 0.5})
    bin_width = 0.05
    alpha = 0.5
    if is_binary(x1, x2):
        sns.histplot(x1, kde=False, ax=ax, label=label1, color='red', alpha=alpha)
        sns.histplot(x2, kde=False, ax=ax, label=label2, color='blue', alpha=alpha)
    else:
        try:
            sns.histplot(x1, ax=ax, label=label1, binwidth=bin_width, color='red', alpha=alpha)
        except RuntimeError:
            print('RuntimeError for x1')
            sns.histplot(x1, ax=ax, label=label1, kde_kws={'bw': bin_width}, binwidth=bin_width, color='red', alpha=alpha)
        try:
            sns.histplot(x2, ax=ax, label=label2, binwidth=bin_width, color='blue', alpha=alpha)
        except RuntimeError:
            print('RuntimeError for x2')
            sns.histplot(x1, ax=ax, label=label1, kde_kws={'bw': bin_width}, binwidth=bin_width, color='blue', alpha=alpha)
    # pass
...
def compare_marginal_qqplots(x1, x2, label1=None, label2=None, ax=None):
    try:
        ...
        # print('qqplot_2samples is not available')
        pass
