...
from BOZORGI_models.preprocess import Preprocess, PlaceHolderTransform
from BOZORGI_models.plotting.plotting import compare_joints, compare_bivariate_marginals
from BOZORGI_utils import T, Y, to_np_vectors, to_np_vector, to_torch_variable, permutation_test, regular_round
...
class BaseGenModel(object, metaclass=BaseGenModelMeta):
    ...
    def __init__(self, w, t, y, train_prop=1, val_prop=0, test_prop=0,
        ...
        self.w_transformed = self.w
        self.t_transformed = self.t
        self.y_transformed = self.y
        self.w_val_transformed = self.w_val
        self.t_val_transformed = self.t_val
        self.y_val_transformed = self.y_val
        self.w_test_transformed = self.w_test
        self.t_test_transformed = self.t_test
    ...
    def plot_ty_dists(self, joint=True, marginal_hist=True, marginal_qq=True,
        ...
        _, t_model, y_model = to_np_vectors(self.sample(seed=seed, untransform=(not transformed), dataset=dataset),
    ...
    def get_univariate_quant_metrics(self, dataset=TRAIN, transformed=False, verbose=True,
                                     thin_model=None, thin_true=None, seed=None, n=None, outcome_distribution=None):
        """
        ...
        _, t_model, y_model = to_np_vectors(
            self.sample(seed=seed, untransform=(not transformed), dataset=dataset),
            thin_interval=thin_model
        )
        ...
        if outcome_distribution.__class__.__name__ == "MixedDistributionAtoms":
            ...
            for index, special_y in enumerate(special_ys):
